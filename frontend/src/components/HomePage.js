...
function HomePage() {
  ...
  const changeLang=(key)=>{
    ...
    if(lang == 1){
     ...
     console.log(data)
     if(data>=0){
      return arr[data].value.slice(0,15);
     }
    ...
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <div className="flex-grow overflow-y-auto bg-black">
        ...
        <div className="relative lg:h-screen lg:flex flex flex-cols lg:justify-between items-center">
          ...
          <h1 className="text-white absolute lg:left-0 left-14 ml-4 lg:text-4xl text-2xl lg:mt-32 mt-72" style={{ fontfamily: "Aclonica", fontWeight: 'semibold' }}>{lang==0?'Video Games Online':'Michezo ya Video Mtandaoni'}</h1>
          ...
          return (
            <div key={trimmedCategoryName} className="bg-black lg:p-9 md:p-5 sm:p-4 p-3">
              ...
              <h1 className="text-2xl lg:p-0 p-2 lg:px-4 lg:py-3 text-amber-400 border-b border-gray-700 bg-zinc-900 rounded-lg font-sans font-bold relative">
                {changeLang(trimmedCategoryName)}{lang == 0 ?'Games':'Michezo'}
              </h1>
        ...
        {!showMore && (
          <div className="flex justify-center items-center bg-black">
            ...
            >
              {lang == 0 ? 'Show More Categories':'Onyesha Kategoria Zaidi'}
            </button>
          </div>
        )}
      </div>
